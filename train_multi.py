...
class BeagleDataset(torch.utils.data.Dataset):
    def __init__(self, imgs_folder_path, csv_file, img_size):
        self.folder2_list = []
        self.label_list = []
        self.folder1_list = []
        self.name_list = []
        ...
        with open(csv_file, 'r') as f:
            ...
            for row in prj_info:
                if prj_info.line_num != 1:
                    labels = list(0 for i in range(24))
                    label_1 = row['label1']
                    label_2 = row['label2']
                    label_3 = row['label3']
                    if len(label_1) != 0:
                        labels[int(float(label_1))] = 1
                    if len(label_2) != 0:
                        labels[int(float(label_2))] = 1
                    if len(label_3) != 0:
                        labels[int(float(label_3))] = 1
                    self.label_list.append(labels)

                    self.folder1_list.append(row['folder1'])
                    self.folder2_list.append(row['folder2'])
                    self.name_list.append(os.path.splitext(row['imagename'])[0])
...
class Net(nn.Module):
    def __init__(self, base_model):
        ...
        base_model.aux1.fc2 = nn.Sequential(
            base_model.aux1.fc2,
            nn.Softmax()
        )
        base_model.aux2.fc2 = nn.Sequential(
            base_model.aux2.fc2,
            nn.Softmax()
        )
...
def val(model_name, model, device, val_loader, command, num_classes, loss_record, accuracy_record, outputdir, criterion, epoch):
        ...
        for data, target in val_loader:
            ...
            for i in range(target.size()[0]):
                ...
                for j in range(num_classes):
                    ...
                    pred = 1 if float(output[i][j]) > 0.5 else 0
                    if label == 1:
                        if label == pred:
                            correct_loose = 1
                    if label == pred:
    ...
    accuracy_comp_e = correct_e / total_n * 100.
    accuracy_comp_l = correct_l / total_n * 100.
    ...
    np.save(os.path.join(val_dir, f'Epoch{epoch}_target.npy'), target_tensor.cpu().numpy())
    np.save(os.path.join(val_dir, f'Epoch{epoch}_output.npy'), output_tensor.cpu().numpy())
    ...
    if epoch == 1:
        ...
        line.extend(list(f'accuracy_{i}' for i in range(num_classes)))
        line.append('overall_accuracy')
        line.append('accuracy_e')
        line.append('accuracy_l')
    ...
    line.append(accuracy_comp_e)
    line.append(accuracy_comp_l)
    with open(os.path.join(outputdir, f'{command}_accuracy.csv'), 'a') as fcsv:
        writer = csv.writer(fcsv)
        writer.writerow(line)
    ...
    with open(os.path.join(outputdir, 'loss_info.txt'), 'a') as ftxt:
        ...
        ftxt.write(f'{command}_accuracy_e: {accuracy_comp_e};')
        ftxt.write(f'{command}_accuracy_l: {accuracy_comp_l}\n')

    if 'test' in command:
        with open(os.path.join(outputdir, 'model_info.txt'), 'a') as ftxt:
            ftxt.write(f'------{command} Result------\n')
            ftxt.write(f'Epoch: {epoch}\tLoss: {val_loss}\tAcc: {accuracy[num_classes]}\tAcc_e: {accuracy_comp_e}\tAcc_l: {accuracy_comp_l}\n')

    print(f'{command} Epoch: {epoch} ' +
          f'Loss: {val_loss:.6f}\tAcc: {accuracy[num_classes]:.4f}%\tAcc_e: {accuracy_comp_e}\tAcc_l: {accuracy_comp_l}')
...
@click.option('--val-batch-size',
              type=int,
              default=200,
              help='batch size for validation and test (default: 64)')
@click.option('--epochs',
              type=int,
              default=200,
...
def main(batch_size, val_batch_size, model_name, epochs):
    ...
    outputdir = os.path.join(os.path.dirname(os.getcwd()), 'beagle_output',
                             f'multi_model.{model_name}-batchsize.{batch_size}')
    os.makedirs(outputdir, exist_ok=True)
    ...
    base_model = getattr(models, model_name)(num_classes=num_classes)
    model = Net(base_model).to(device)
    optimizer = optim.Adam(model.parameters(), weight_decay=1e-4, amsgrad=False)
